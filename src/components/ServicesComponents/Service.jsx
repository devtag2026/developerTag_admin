...
const ServiceDashboard = () => {
    ...
    const { state, fetchServices, setSearchQuery } = useService();
    const [currentPage, setCurrentPage] = useState(1);
    const [searchInput, setSearchInput] = useState('');

    useEffect(() => {
        fetchServices({ page: currentPage, limit: 3, search: state.searchQuery });
    ...
    const handleCreateNew = () => {
        router.push('/admin/services/create-service');
    };
    ...
    if (state.isLoading && state.services.length === 0) {
        return (
    ...
    return (
        <div className="max-w-7xl mx-auto p-6 bg-white">
            <div className="flex justify-between items-center mb-8 pb-6 border-b-2 border-[#00bba7]">
                <h1 className="text-3xl font-semibold text-gray-800">Services Management</h1>
                <button
                ...
                {state.services.length === 0 ? (
                    <div className="col-span-full text-center py-16">
                        <h3 className="text-xl font-medium text-gray-800 mb-2">
                            {state.searchQuery ? 'No services found' : 'No services created yet'}
                        </h3>
